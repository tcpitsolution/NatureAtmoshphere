...
import showroom from '../assets/images/showroom.jpeg'
...
export default function Home() {
  return (
    <>
      ...
      <section style={{ position: 'relative', minHeight: '92vh', display: 'flex', alignItems: 'center', color: 'var(--color-ivory)', overflow: 'hidden' }}>
        ...
        <div
          style={{
            position: 'absolute',
            inset: 0,
            background: 'linear-gradient(100deg, rgba(33,29,27,0.94) 12%, rgba(33,29,27,0.55) 48%, rgba(110,20,35,0.35) 100%)',
          }}
        ></div>
        <div className="container" style={{ position: 'relative', zIndex: 2 }}>
          <div className="eyebrow">Varanasi, India · Est. Handicraft &amp; Wellness House</div>
          <h1 style={{ fontSize: 'clamp(40px,6vw,76px)', maxWidth: '14ch', lineHeight: 1.08, color: 'var(--color-ivory)', marginTop: 26 }}>
            Discover Nature.<br />
            <em className="accent-script" style={{ color: 'var(--color-gold-light)' }}>Experience</em> Wellness.
          </h1>
          <p style={{ marginTop: 26, maxWidth: '44ch', fontSize: 16.5, lineHeight: 1.75, color: 'var(--color-ivory-dim)' }}>
      ...
      {/* INTRO STRIP */}
      <div style={{ background: 'var(--color-charcoal)', color: 'var(--color-ivory)', padding: '70px 32px', textAlign: 'center' }}>
        <div className="accent-script" style={{ fontSize: 22, color: 'var(--color-gold-light)' }}>
          "Bringing the finest handcrafted &amp; natural products from Varanasi to the world."
        </div>
        <p style={{ maxWidth: '60ch', margin: '18px auto 0', color: 'var(--color-ivory-dim)', fontSize: 14.5 }}>
      ...
      <section className="section">
        ...
        <div className="container" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 1, background: 'var(--color-line)', border: '1px solid var(--color-line)' }}>
          <WhyCard label="Authentic" title="Genuine Indian Craft" text="Every piece is sourced and handmade using traditional techniques passed through generations." />
          <WhyCard label="Premium" title="Handmade Quality" text="Carefully selected materials — from pure oils to fine silver — with no shortcuts taken." />
          <WhyCard label="Global" title="Worldwide Shipping" text="Trusted by customers across India and internationally, with secure delivery to your door." />
          <WhyCard label="Secure" title="Safe Online Payment" text="Multiple trusted payment options and encrypted checkout for complete peace of mind." />
        </div>
      </section>
      ...
      <section className="section" id="categories" style={{ background: 'var(--color-ivory-dim)' }}>
        ...
        />
        <div className="container" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 26 }}>
          {categories.map((c) => (
            <CategoryCard key={c.title} tag={c.tag} title={c.title} image={c.image} />
          ))}
        </div>
      </section>
      ...
      <section className="container section" style={{ display: 'grid', gridTemplateColumns: '0.9fr 1.1fr', gap: 80, alignItems: 'center' }}>
        <div style={{ position: 'relative' }}>
          <img src={showroom} alt="Inside the Nature Atmosphere showroom" style={{ width: '100%', height: 480, objectFit: 'cover' }} />
        </div>
        <div>
      ...
      <section className="section" style={{ background: 'var(--color-charcoal)', color: 'var(--color-ivory)', textAlign: 'center' }}>
        ...
        <div className="container" style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 14 }}>
          {paymentMethods.map((p) => (
            <PayChip key={p} label={p} />
          ))}
        </div>
      </section>

      {/* CTA BAND */}
      <section style={{ background: 'var(--color-maroon)', color: 'var(--color-ivory)', textAlign: 'center', padding: '100px 32px' }}>
        <div className="accent-script" style={{ fontSize: 26, color: 'var(--color-gold-light)' }}>
          Have a question about our products?
        </div>
        <h2 style={{ fontSize: 'clamp(28px,3.6vw,44px)', marginTop: 16, color: 'var(--color-ivory)' }}>Send Us an Inquiry</h2>
        <p style={{ maxWidth: '50ch', margin: '20px auto 0', color: 'rgba(247,241,230,0.8)' }}>
